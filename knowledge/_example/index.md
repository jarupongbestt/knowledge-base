...
|---|---|
| payment retries timing out silently | `retry-backoff.md` |

## Navigation Hints — Sources-derived (locked, mirrors a raw file in sources/)

| When working on... | Read this page | Raw source |
|---|---|---|
| exact fields/values the payment gateway's webhook sends | `field-reference.md` | `sources/payment-gateway/openapi.yaml` |

## Overview

A payment integration domain. Both pages below live flat, directly under
`knowledge/_example/`. What distinguishes them is their frontmatter, not their
location:

- `retry-backoff.md` has no `source_ref` — it's a lesson the team learned while
...
- `field-reference.md` has `source_ref: knowledge/_example/sources/payment-gateway/openapi.yaml`
  and `locked: true` — it's a straight mirror of that raw file (sources-derived; only
...
`field-reference.md` against `sources/payment-gateway/openapi.yaml` directly: the raw
file is untouched YAML with no prose; the knowledge page is its readable, faithful
mirror. Never confuse the two — a raw file is not knowledge, and a knowledge page is
not a raw file.
